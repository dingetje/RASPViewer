 ...
 *
 * 03-Jan-2020
 * - modified parameter description handling
 *
 * 16-Mar-2020
...
//  var Now = new Date(2019,3,16).getTime();  // Time now - in milliSec(!)
  ...
  T.setTime(Now - 7*mS_Day);  // NL-7
  day.options[0] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now - 6*mS_Day);  // NL-6
  day.options[1] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now - 5*mS_Day);  // NL-5
  day.options[2] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now - 4*mS_Day);  // NL-4
  day.options[3] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now - 3*mS_Day);  // NL-3
  day.options[4] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now - 2*mS_Day);  // NL-2
  day.options[5] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now - 1*mS_Day);  // NL-1
  day.options[6] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));

  T.setTime(Now);         // Today
  day.options[7] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()] + " - Today", mkdat(T));
  T.setTime(Now + mS_Day);  // Tomorrow
  day.options[8] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now + 2 * mS_Day);
  day.options[9] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now + 3 * mS_Day);
  day.options[10] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
  T.setTime(Now + 4 * mS_Day);
  day.options[11] = new Option(dayName[T.getDay()] + ' ' + T.getDate() + ' ' + monthName[T.getMonth()], mkdat(T));
...
//  $("#paramTooltip").attr('data-tooltip', paramListLite[document.getElementById("Param").selectedIndex][3]);
  ...
  L.easyButton('<i id="parameterDescriptionButtonOnMap" class="fas fa-info-circle fa-lg" title="Parameter Description"></i>',
    function(btn, map){ 
  ...
  new LongClick(map, 1000); // instatiate longclick
...
/* show parameter description on request */
function showParameterDescription()
{
   if (parameterDescription)
   {
      // Notiflix Report - Basic Initialize
      if (isSmallScreen())
      {
      ...
      Notiflix.Report.Info(parameterName,parameterDescription,"Close");
   }
}
  ...
  */
  ...
  if (req) {
	...
	if(infoType != "XBL") {
	  ...
	  hideLoading();
	  // grab the img line from the output
          var re = /<img[^>]+src='https:\/\/([^'>]+)/g
          var results = re.exec(req.responseText);
	  if(results){
            window.open("https://" + results[1], '_blank').focus();
	    console.log(req.responseText);
	  }
	  else {
		alert("Sorry, failed to create XBL plot, maybe no data?");
	  }
          //addInfo(Event.latlng, req.responseText, infoType);
        }
      }
    }
...
//  }
  ...
  if(oldParam !== document.getElementById("Param").value) {
      ...
      parameterName=paramListFull[document.getElementById("Param").selectedIndex][2];
		  ...
		  parameterName=paramListLite[document.getElementById("Param").selectedIndex][2];
	  }
  }
  /* Clear saved images
...
function getBasedir()
{
  ...
  switch(document.getElementById("Day").selectedIndex){
    case 0: basedir = "NL-7"; break;
    case 1: basedir = "NL-6"; break;
    case 2: basedir = "NL-5"; break;
    case 3: basedir = "NL-4"; break;
    case 4: basedir = "NL-3"; break;
    case 5: basedir = "NL-2"; break;
    case 6: basedir = "NL-1"; break;
    case 7: basedir = "NL+0"; break;
    case 8: basedir = "NL+1"; break; 
    case 9: basedir = "NL+2"; break;
    case 10: basedir = "NL+3"; break;
    case 11: basedir = "NL+4"; break;
...
function addInfo(location, txt, infoType)
{
  ...
  //  deleteInfo();
  if(infoType != "XBL") { 
  	var nlines = 3; // Always need at least two lines to  prevent scroll bars
  	var ind = 0;
  	var longline = false;
	...
	var infoWindow = new L.popup( { maxWidth: imgWid/3, minWidth: 200 })
  	...
  	var infoWindow = new L.popup( { "minWidth": "520", "keepInView": "true" })
            .setLatLng(location)
            .setContent(txt)
            .openOn(map);
  	infoWindow.on('remove', function(e) { popupClosed(e) }); 
...
function nextDay()
{
  ...
  index++;
  if (index >= SAVE_DAYS + HISTORY_DAYS) {
    index = 0;
  }
...
function prevDay()
{
  ...
  index--;
  if (index < 0) {
    index = HISTORY_DAYS + SAVE_DAYS - 1;
  }
  document.getElementById("Day").options[index].selected = true;
  doChange();
}
