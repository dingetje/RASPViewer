...
*/
  ...
  //                                       0          1           2
  map = new L.map('zoomBox',{layers:[ownLayer, tonerLayer, terrainLayer]});
...
function addSndMarkers()
{
  ...
  for(i = 1; i < soundings.LOC.length; i++ ){ // No Sounding0 !!
    ...
    var marker = L.marker(soundings.LOC[i], {icon: sndIcon, myId: i}).addTo(map);
...
function setSize()
{
  ...
  imgWid -= ((document.getElementById("selectors").offsetWidth) + 10);
  imgHt  -= 6;
...
function doChange(E)
{
  ...
  /*  Descriptions are tooltips now */
  if(fullSet){
    $("#paramTooltip").attr('data-tooltip', paramListFull[document.getElementById("Param").selectedIndex][3]);
  }
  else {
    $("#paramTooltip").attr('data-tooltip', paramListLite[document.getElementById("Param").selectedIndex][3]);
  }
