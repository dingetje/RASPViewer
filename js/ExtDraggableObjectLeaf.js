 ...
 *     around the page.
 * @added support for touch events. June 2, 2012 - Alan Crouse
 */
...
function ExtDraggableObject(src, opt_drag) {
  ...
  mouseDownEvent_ = event_.addListener(src, "mousedown", mouseDown_);
  mouseUpEvent_ = event_.addListener(target_, "mouseup", mouseUp_);
  touchStartEvent_ = event_.addListener(src, "touchstart", mouseDown_);
  touchEndEvent_ = event_.addListener(target_, "touchend", mouseUp_);
  ...
  function mouseUp_(ev) {
    ...
    if (moving_) {
      ...
      event_.removeListener(touchMoveEvent_);
